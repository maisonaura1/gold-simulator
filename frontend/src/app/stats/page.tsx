        ...
        {loading ? (
          ...
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-4 gap-2">
              <MTStatBox label={t.winrateLabel} value={`${stats.winRate}%`} cls={stats.winRate >= 50 ? 'text-[var(--mt-green)]' : 'text-[var(--mt-red)]'} />
              <MTStatBox label={t.totalPL} value={`${stats.totalPnl >= 0 ? '+' : ''}$${stats.totalPnl.toFixed(2)}`} cls={stats.totalPnl >= 0 ? 'text-[var(--mt-green)]' : 'text-[var(--mt-red)]'} />
              <MTStatBox label={t.totalTrades} value={String(stats.totalTrades)} cls="text-[var(--mt-cyan)]" />
              <MTStatBox label={t.avgPL} value={`${stats.avgPnl >= 0 ? '+' : ''}$${stats.avgPnl.toFixed(2)}`} cls={stats.avgPnl >= 0 ? 'text-[var(--mt-green)]' : 'text-[var(--mt-red)]'} />
              <MTStatBox label={t.avgRisk} value={`${stats.avgRisk.toFixed(2)}%`} cls={stats.avgRisk <= 2 ? 'text-[var(--mt-green)]' : 'text-[var(--mt-yellow)]'} />
              <MTStatBox label={t.avgRR} value={`${stats.avgRR.toFixed(2)}:1`} cls={stats.avgRR >= 2 ? 'text-[var(--mt-green)]' : 'text-[var(--mt-yellow)]'} />
