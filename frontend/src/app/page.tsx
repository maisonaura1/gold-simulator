...
const FEATURES = [
  { icon: '◈', title: 'Real XAUUSD Data',        body: 'Practice on historical gold spot prices with realistic bid/ask spreads and live market replay.' },
  { icon: '◎', title: 'Risk Calculator',          body: 'Calculate position size, stop-loss, take-profit and R:R ratio before every trade. Build discipline.' },
  { icon: '▦', title: 'Performance Analytics',   body: 'Trader Score, win rate, drawdown, equity curve and behaviour analysis after every session.' },
  { icon: '⬡', title: 'Zero Capital at Risk',    body: 'Use virtual capital. Learn real trading mechanics without putting real money on the line.' },
  { icon: '◇', title: 'XAUUSD Only Focus',       body: 'Full focus on gold spot. One instrument, mastered deeply — the way professional desks operate.' },
  { icon: '◉', title: 'Missions & XP',           body: 'Structured learning paths, daily missions and XP rewards to build consistency over time.' },
];

const STEPS = [
  { n: '01', title: 'Create your free account', body: 'Sign up in 30 seconds. No credit card required. Instant access to 10 free simulations.' },
  { n: '02', title: 'Load a gold session',       body: 'Pick a historical scenario or replay live XAUUSD data from real market sessions.' },
  { n: '03', title: 'Trade and review',          body: 'Enter positions, set SL/TP, review your Trader Score and analytics after each session.' },
];
...
const TIERS = [
  {
    ...
    features: ['10 trade simulations included', 'Basic stats: win rate & P/L', 'XAUUSD chart & replay', 'Risk calculator', 'Community missions'],
    locked:   ['Full performance analytics', 'Equity curve & drawdown', 'Trader Score & coaching', 'Unlimited sessions', 'Trade journal & export'],
  },
  {
    name: 'Pro', price: '€9.95', period: 'one-time · lifetime access', highlight: true,
    cta: 'Unlock everything', href: '/auth/register?plan=pro',
    features: ['Unlimited trade simulations', 'Full analytics dashboard', 'Equity curve & max drawdown', 'Trader Score + coaching tips', 'Trade journal & export', 'All missions unlocked', 'Priority support'],
    locked: [],
  },
];

const TESTIMONIALS = [
  { name: 'Daan V.',    flag: '🇳🇱', role: 'Retail trader, 8 mo.',   text: 'My win rate went from 38% to 57% in 6 weeks of daily practice. The Trader Score feedback is brutal but honest.' },
  { name: 'Sophie M.',  flag: '🇩🇪', role: 'Beginner, 3 mo.',         text: 'Finally a simulator that forces me to think about risk BEFORE entering a trade. The R:R calculator changed everything.' },
  { name: 'Luca B.',    flag: '🇮🇹', role: 'Prop trader applicant',   text: 'Used this to prep for my FTMO challenge. Passed on the first try. The blotter workflow is exactly like a real desk.' },
  { name: 'Emma R.',    flag: '🇬🇧', role: 'Swing trader, 1 yr.',     text: 'At €9.95 for lifetime access this is ridiculous value. I\'ve spent more on trading books that gave me less.' },
];

const FAQS = [
  { q: 'Is this a real brokerage or demo account?',         a: 'Neither — it\'s a pure simulator. No real money, no broker connection. You practice using historical XAUUSD data in a safe environment.' },
  { q: 'What happens after I use my 10 free simulations?',  a: 'You\'ll see a prompt to upgrade. The €9.95 one-time payment unlocks unlimited sessions forever — no recurring fees.' },
  { q: 'Is the market data real?',                          a: 'Yes. We use real historical XAUUSD spot price data with authentic bid/ask spreads. Scenarios are replayed at market speed.' },
  { q: 'Can I track my progress over time?',                a: 'Pro users get a full analytics dashboard with equity curve, drawdown history, Trader Score trend and session-by-session breakdown.' },
  { q: 'Do I need trading experience to start?',            a: 'No. The missions and academy section cover everything from reading a candle chart to building a complete trading system.' },
  { q: 'Is there a mobile app?',                            a: 'The web app is fully responsive. A dedicated iOS/Android app is on the roadmap for Q4 2026.' },
];
...
function PricingCard({ tier }: { tier: typeof TIERS[0] }) {
  return (
    <div className="p-6 rounded-sm flex flex-col" style={{
      ...
      <div style={{ color: tier.highlight ? '#e8c96d' : '#c8cdd8', fontSize: 32, fontWeight: 800, fontFamily: 'monospace', lineHeight: 1 }}>{tier.price}</div>
      ...
      <Link href={tier.href} className="block text-center py-2.5 rounded-sm text-sm font-bold tracking-wide" style={{
...
function FakeTicker() {
  ...
  const bid = (price - 0.10).toFixed(2);
  const ask = (price + 0.10).toFixed(2);
  ...
  return (
    <div className="flex items-center justify-center gap-6 flex-wrap mt-6 mb-2">
      ...
      <div className="flex items-center gap-3 font-mono text-xs" style={{ color: '#6b7385' }}>
        <span>BID <span style={{ color: '#2dcc6f' }}>{bid}</span></span>
        <span>ASK <span style={{ color: '#e84040' }}>{ask}</span></span>
  ...
  return (
    <div className="relative mx-auto max-w-4xl mt-12 mb-2" style={{ perspective: '1200px' }}>
      ...
      <div style={{
        ...
        boxShadow: '0 40px 80px rgba(0,0,0,0.6), 0 0 0 1px #1d2029',
      }}>
        {/* Browser chrome */}
        ...
        <div className="flex" style={{ height: 260 }}>
          ...
          <div className="flex flex-col shrink-0 p-3 gap-2" style={{ width: 180, background: '#0b0d11' }}>
            ...
            <div className="text-center py-1.5 rounded text-xs font-bold mt-1"
              style={{ background: 'linear-gradient(135deg,#2c2410,#1a1508)', color: '#c9a84c', border: '1px solid #c9a84c44' }}>
              ◆ DRAFT BUY
            </div>
            <div className="text-center text-xs" style={{ color: '#6b7385', fontSize: 9 }}>Notional $33,425.00</div>
          </div>
        </div>
        ...
        <div style={{ borderTop: '1px solid #1d2029', background: '#09090d' }}>
          ...
          {[
            { side: 'BUY',  oz: '10.0', price: '3342.50', status: 'APPROVED', notional: '$33,425' },
            { side: 'SELL', oz: '5.0',  price: '3338.20', status: 'SUBMITTED', notional: '$16,691' },
            { side: 'BUY',  oz: '20.0', price: '3310.00', status: 'DRAFT',    notional: '$66,200' },
          ].map((o, i) => (
            <div key={i} className="flex items-center gap-4 px-3 py-1.5 font-mono text-xs" style={{ borderBottom: '1px solid #0b0d11' }}>
              <span style={{ color: o.side === 'BUY' ? '#2dcc6f' : '#e84040', fontWeight: 700, minWidth: 36 }}>
                {o.side === 'BUY' ? '▲' : '▼'} {o.side}
              </span>
              <span style={{ color: '#8893a8' }}>XAUUSD</span>
              <span style={{ color: '#8893a8' }}>{o.oz} oz</span>
              <span style={{ color: '#8893a8' }}>@ {o.price}</span>
              <span style={{ color: '#6b7385' }}>{o.notional}</span>
              <span style={{
                color: o.status === 'APPROVED' ? '#2dcc6f' : o.status === 'SUBMITTED' ? '#f0b429' : '#6b7385',
...
function TestimonialCard({ name, flag, role, text }: typeof TESTIMONIALS[0]) {
  return (
    <div className="p-5 rounded-sm flex flex-col gap-3" style={{ background: '#0f1117', border: '1px solid #1d2029' }}>
      <div style={{ color: '#6b7385', fontSize: 12, lineHeight: 1.7 }}>"{text}"</div>
      <div className="flex items-center gap-2 mt-auto pt-2" style={{ borderTop: '1px solid #1d2029' }}>
        ...
        <div>
          <div style={{ color: '#c8cdd8', fontSize: 12, fontWeight: 600 }}>{name}</div>
          <div style={{ color: '#6b7385', fontSize: 10 }}>{role}</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ borderBottom: '1px solid #1d2029' }}>
      ...
      >
        ...
        <span style={{ color: '#c9a84c', fontSize: 14, flexShrink: 0, transition: 'transform 0.2s', transform: open ? 'rotate(45deg)' : 'rotate(0)' }}>+</span>
      </button>
      {open && (
        <div className="pb-4" style={{ color: '#6b7385', fontSize: 12, lineHeight: 1.7 }}>{a}</div>
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen font-sans" style={{ background: '#07080b', color: '#c8cdd8' }}>
      ...
      <nav className="flex items-center justify-between px-6 py-3 border-b sticky top-0 z-40"
        ...
        <div className="hidden sm:flex items-center gap-6 text-xs" style={{ color: '#6b7385' }}>
          <a href="#features" style={{ color: '#6b7385', textDecoration: 'none' }}>Features</a>
          <a href="#pricing"  style={{ color: '#6b7385', textDecoration: 'none' }}>Pricing</a>
          <a href="#faq"      style={{ color: '#6b7385', textDecoration: 'none' }}>FAQ</a>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-10 text-center">
        <GoldLabel>XAUUSD Gold Spot Simulator</GoldLabel>
        ...
        <p className="max-w-xl mx-auto mb-8"
          ...
          The professional XAUUSD simulator for traders who want to sharpen entries, exits and risk management — without risking real capital.
        </p>
        <div className="flex items-center justify-center gap-3 flex-wrap">
          <Link href="/auth/register"
            className="px-7 py-3 rounded-sm font-bold text-sm"
            style={{ background: 'linear-gradient(135deg,#c9a84c,#a8893c)', color: '#000', textDecoration: 'none' }}>
        ...
        <p style={{ color: '#3a3f4d', fontSize: 11, marginTop: 12 }}>
          No credit card required · 10 free simulations included · Instant access
        </p>
        <FakeTicker />
      </section>
      ...
      <section id="preview" className="max-w-5xl mx-auto px-6 py-16">
        <div className="text-center mb-2">
          ...
          <p style={{ color: '#6b7385', fontSize: 13, marginTop: 6 }}>
            Gold ticker, order blotter, audit workflow — exactly how institutional desks operate.
          </p>
        </div>
        <ProductPreview />
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 py-16">
        <h2 className="text-center font-bold mb-2" style={{ color: '#e8ecf4', fontSize: 22 }}>What traders say</h2>
        <p className="text-center mb-10" style={{ color: '#6b7385', fontSize: 13 }}>Real feedback from traders using the simulator daily.</p>
      ...
      <section id="pricing" className="max-w-3xl mx-auto px-6 py-16">
        <h2 className="text-center font-bold mb-2" style={{ color: '#e8ecf4', fontSize: 22 }}>Simple pricing</h2>
        <p className="text-center mb-10" style={{ color: '#6b7385', fontSize: 13 }}>
        ...
        <p className="text-center mt-6" style={{ color: '#3a3f4d', fontSize: 11 }}>
          One-time payment · Lifetime access · iDEAL & card accepted · 30-day refund guarantee
        </p>
      </section>
      ...
      <section className="border-t py-14 text-center" style={{ borderColor: '#1d2029', background: '#09090d' }}>
        ...
        <div className="flex items-center justify-center gap-6 mt-8 flex-wrap">
          {['✓ No credit card', '✓ Instant access', '✓ 30-day refund', '✓ Lifetime Pro deal'].map((t) => (
      ...
      <footer className="border-t px-6 py-6 flex items-center justify-between flex-wrap gap-4"
        ...
        <div className="flex gap-5">
          {[['Log in', '/auth/login'], ['Register', '/auth/register'], ['Pricing', '#pricing'], ['FAQ', '#faq']].map(([l, h]) => (
            <a key={l} href={h} style={{ color: '#3a3f4d', fontSize: 11, textDecoration: 'none' }}>{l}</a>
          ))}
        </div>
      </footer>
    </div>
